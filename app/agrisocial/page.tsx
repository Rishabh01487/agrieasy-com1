...
function PostCard({ post, viewerId, onLike, onDelete }: { post: Post; viewerId: string; onLike: (id: string, liked: boolean, count: number) => void; onDelete?: (id: string) => void }) {
    ...
    return (
        <div className="fade-in-up" style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(232,123,123,0.2)', borderRadius: '12px', overflow: 'hidden', marginBottom: '16px', boxShadow: SHARED.shadowMd }}>
            {/* Header */}
                        ...
                        {viewerId !== authorId && authorId && (
                            <button
                                ...
                                style={{
                                    ...
                                    boxShadow: following
                                        ? 'inset 0 1px 0 rgba(255,255,255,0.5), 0 1px 3px rgba(232,123,123,0.15)'
                                        : 'inset 0 1px 0 rgba(255,255,255,0.3), 0 2px 6px rgba(232,123,123,0.25)',
...
function SuggestedSidebar({ users, viewerId }: { users: SuggestedUser[]; viewerId: string }) {
    ...
    return (
        <div style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(232,123,123,0.2)', borderRadius: '12px', padding: '16px', boxShadow: SHARED.shadowMd }}>
            ...
            <div className="agrisocial-layout" style={{ maxWidth: '1100px', margin: '0 auto', padding: '16px 14px 80px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: '24px' }}>
                ...
                <div style={{ minWidth: 0, maxWidth: '600px', margin: '0 auto', width: '100%' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', overflowX: 'auto' }} className="no-scrollbar">
                        {([['following', 'Following'], ['ranked', '🔥 Top'], ['latest', '⏱ Latest']] as const).map(([k, label]) => (
                            <button key={k} onClick={() => setFeed(k)}
                                style={{ padding: '7px 16px', background: feed === k ? 'linear-gradient(135deg, rgba(232,123,123,0.55), rgba(232,123,123,0.35))' : 'rgba(255,255,255,0.6)', border: `1.5px solid ${feed === k ? SOCIAL.primary : SOCIAL.border}`, borderRadius: '100px', fontSize: '0.78rem', fontWeight: 700, color: feed === k ? '#fff' : SOCIAL.textSecondary, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                {label}
                            </button>
                        ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '14px', marginBottom: '6px' }} className="no-scrollbar">
                        {CATEGORIES.map(c => (
                            <button key={c.key} onClick={() => setCategory(c.key)}
                                style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '7px 14px', background: category === c.key ? 'linear-gradient(135deg, rgba(232,123,123,0.55), rgba(232,123,123,0.35))' : 'rgba(255,255,255,0.6)', border: `1.5px solid ${category === c.key ? SOCIAL.primary : SOCIAL.border}`, borderRadius: '100px', fontSize: '0.78rem', fontWeight: 700, color: category === c.key ? '#fff' : SOCIAL.textSecondary, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                {c.icon} {c.label}
                            </button>
                        ))}
                    </div>
                    ...
                    <div onClick={() => router.push('/agrisocial/create')}
                        style={{ background: 'rgba(255,255,255,0.6)', border: '1.5px solid rgba(232,123,123,0.2)', borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', cursor: 'pointer', boxShadow: SHARED.shadowMd }}>
                    ...
                    {loading ? (
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(232,123,123,0.2)', borderRadius: '12px', padding: '40px 24px', textAlign: 'center', boxShadow: SHARED.shadowMd }}>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(232,123,123,0.2)', borderRadius: '12px', padding: '48px 24px', textAlign: 'center', boxShadow: SHARED.shadowMd }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {userId && (
                        <Link href={`/agrisocial/profile/${userId}`} style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(232,123,123,0.2)', borderRadius: '12px', padding: '14px', display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none', boxShadow: SHARED.shadowMd }}>
