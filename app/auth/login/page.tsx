...
// signIn import removed — Google sign-in temporarily disabled.
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (params.get('registered') === '1') {
      // Google sign-in is temporarily disabled — show a single success message
      // for all newly-registered users (manual registration only).
      setSuccess('Account created! Please sign in with your email/phone and password.')
    }
  }, [])
    ...
    <div style={{ minHeight: '100vh', background: AUTH.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: SHARED.font, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: AUTH.white, borderRadius: '24px', padding: '40px', width: '100%', maxWidth: '420px', boxShadow: SHARED.shadowXl, border: `1px solid transparent`, backgroundImage: `linear-gradient(${AUTH.white}, ${AUTH.white}), ${AUTH.gradient}`, backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' }}>
        ...
        {/* Google Sign In — TEMPORARILY DISABLED
            Google OAuth is in branding verification with Google Cloud.
            Until that's resolved, we hide the Google button and show a
            notice. Users can still sign in via the manual form below.
            To re-enable: restore the onClick={signIn('google', ...)} button
            and remove this notice block. */}
        <div style={{
          width: '100%',
          padding: '12px 14px',
          borderRadius: '12px',
          ...
          <svg width="20" height="20" viewBox="0 0 24 24" style={{ opacity: 0.4, flexShrink: 0 }}>
          ...
          <div>
            <div style={{ marginBottom: '2px' }}>Google sign-in is temporarily disabled</div>
            <div style={{ fontSize: '0.74rem', fontWeight: 500, color: '#9CA3AF' }}>
              Please sign in with your email/phone and password below.
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
          ...
          <span style={{ color: AUTH.muted, fontSize: '0.78rem', fontFamily: SHARED.font }}>use email</span>
          <div style={{ flex: 1, height: '1px', background: AUTH.border }} />
        </div>
