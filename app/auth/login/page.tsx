...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (params.get('registered') === '1') {
      // Google users: tell them to click "Continue with Google" to sign in.
      // Manual users: just tell them their account was created.
      const isGoogle = params.get('google') === '1'
      setSuccess(isGoogle ? 'Account created! Click "Continue with Google" below to sign in.' : 'Account created! Please sign in.')
    }
  }, [])
  ...
  return (
    <div style={{ minHeight: '100vh', background: AUTH.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: SHARED.font, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: AUTH.white, borderRadius: '24px', padding: '40px', width: '100%', maxWidth: '420px', boxShadow: SHARED.shadowXl, border: `1px solid transparent`, backgroundImage: `linear-gradient(${AUTH.white}, ${AUTH.white}), ${AUTH.gradient}`, backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' }}>
        ...
        <button onClick={() => signIn('google', { callbackUrl: '/auth/login' })} style={{ width: '100%', padding: '11px', borderRadius: '12px', background: AUTH.bg, border: `1.5px solid ${AUTH.border}`, color: AUTH.text, fontSize: '0.93rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '20px', fontFamily: SHARED.font, transition: 'background 0.2s, border-color 0.2s' }}>
