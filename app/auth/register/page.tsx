  ...
  return (
    <div style={{ minHeight: '100vh', background: AUTH.bg, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 24px', fontFamily: SHARED.font, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: AUTH.white, borderRadius: '24px', padding: '36px', width: '100%', maxWidth: '460px', boxShadow: SHARED.shadowXl, border: `1px solid transparent`, backgroundImage: `linear-gradient(${AUTH.white}, ${AUTH.white}), ${AUTH.gradient}`, backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' }}>
        ...
        {/* Google Sign Up — pass the selected role via authorization params
            so the NextAuth signIn callback can create the user with the correct role */}
        <button onClick={() => signIn('google', {
          callbackUrl: `/${role}/dashboard`,
          authorization: { params: { role } }
        })} style={{ width: '100%', padding: '11px', borderRadius: '12px', background: AUTH.bg, border: `1.5px solid ${AUTH.border}`, color: AUTH.text, fontSize: '0.93rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '16px', fontFamily: SHARED.font, transition: 'background 0.2s, border-color 0.2s' }}>
