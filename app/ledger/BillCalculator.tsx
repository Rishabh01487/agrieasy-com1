...
export default function BillCalculator({ embedded = false, onSaved, initialResult = null }: BillCalculatorProps) {
    ...
    const [billed, setBilled] = useState(false)  // true after bill is saved/printed/shared — locks editing
    const [paymentMode, setPaymentMode] = useState<'cash' | 'upi' | 'bank' | 'due'>('cash')
                    ...
                    data = {
                        commodities: (ocrData.commodities || []).map((c: any) => ({
                            ...
                            batches: (c.batches || []).map((b: any) => ({
                                bagCount: b.bagCount,
                                weight: b.weight,
                                individualWeights: b.individualWeights || [],
                            })),
    ...
    /** Update a single individual bag weight (e.g. OCR read 55.0 as 55.5 — user fixes it).
     *  Also recalculates the batch total weight + commodity totals + grand totals. */
    const updateIndividualWeight = (commodityIdx: number, batchIdx: number, weightIdx: number, value: string) => {
        if (!result) return
        ...
        const newCommodities = result.commodities.map((c, i) => {
            ...
            const newBatches = c.batches.map((b, j) => {
                if (j !== batchIdx) return b
                if (!b.individualWeights) return b
                const newWeights = [...b.individualWeights]
                newWeights[weightIdx] = isNaN(num) || num < 0 ? 0 : Number(num.toFixed(3))
                // Recalculate batch total from individual weights
                const newBatchWeight = Number(newWeights.reduce((s, w) => s + w, 0).toFixed(3))
                return { ...b, individualWeights: newWeights, weight: newBatchWeight, bagCount: newWeights.length }
            })
            const totalBags = newBatches.reduce((s, b) => s + b.bagCount, 0)
            const totalWeight = Number(newBatches.reduce((s, b) => s + b.weight, 0).toFixed(3))
            return { ...c, batches: newBatches, totalBags, totalWeight }
        ...
        try {
            ...
            const res = await authFetch('/api/ledger', {
                ...
                body: JSON.stringify({
                    ...
                    status: paymentMode === 'due' ? 'partial' : 'paid',
                    paymentMode,
                    remainingAmount: paymentMode === 'due' ? Number(grandTotalAmount.toFixed(2)) : 0,
                    locked: true,
                }),
            })
            const d = await res.json()
            if (res.ok) {
                setSaveMsg('✓ Bill saved & locked. Payment mode: ' + paymentMode.toUpperCase() + (paymentMode === 'due' ? ' — track installments on ledger page' : ''))
                setBilled(true)  // lock the bill — no more editing
                onSaved?.()
    ...
    const buildReceiptHtml = () => {
        ...
        const rowsHtml = computedRows.map((c, i) => {
            // Render all batches for this commodity side-by-side in a single row.
            // Each batch is a column with: batch header + individual weights + subtotal.
            // This saves vertical space when there are many batches/commodities.
            const batchesSideBySide = c.batches.map((b, j) => {
                const weightsHtml = b.individualWeights && b.individualWeights.length > 0
                    ? b.individualWeights.map((w) => `<div>${w.toFixed(2)}</div>`).join('')
                    : ''
                return `
                    <td style="padding: 4px 6px; background: #FAFAF5; border: 1px solid #EFE6DC; vertical-align: top; width: ${Math.floor(100 / c.batches.length)}%;">
                        <div style="font-size: 10px; color: #AC3B61; font-weight: 700; margin-bottom: 4px; border-bottom: 1px solid #EFE6DC; padding-bottom: 3px;">
                            Batch ${j + 1}: ${b.bagCount} bags
                        </div>
                        <div style="font-size: 11px; color: #2A2A2A; line-height: 1.5; font-family: monospace;">
                            ${weightsHtml}
                        </div>
                        <div style="font-size: 10px; color: #AC3B61; font-weight: 700; margin-top: 4px; padding-top: 3px; border-top: 1px dashed #EFE6DC;">
...
${computedRows.map((c, i) => {
            const batchLines = c.batches.map((b, j) => {
                ...
                const bagLines = b.individualWeights && b.individualWeights.length > 0
                    ? b.individualWeights.map((w) => `   ${w.toFixed(2)}`).join('\n')
                    : ''
...
<script>
    ...
    const printReceipt = () => { openReceipt(); setBilled(true) }  // lock after print

    // Native share from the main page (without opening new window)
    const shareReceipt = async () => {
        if (!result || computedRows.length === 0) return
        setBilled(true)  // lock after share
        const now = new Date()
        const receiptNo = 'AG-' + now.getTime().toString().slice(-8)
        const dateStr = now.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
        const shareText = `*AgriEasy Bill Receipt*
...
Generated by AgriEasy · Jai Jawan, Jai Kisan 🇮🇳`
    ...
    const resetAll = () => {
        setResult(null); setFile(null); setPreviewUrl(''); setRates({}); setSaveMsg(''); setError(''); setBillPhotoUrl(''); setBilled(false); setPaymentMode('cash')
    }
            ...
            {result && (
                <div>
                        ...
                        {computedRows.map((c, i) => (
                            <div key={i} style={{ background: palette.white, border: `1px solid ${palette.borderLight}`, borderRadius: 12, padding: 16, boxShadow: SHARED.shadow }}>
                                ...
                                <div style={{ borderTop: `1px solid ${palette.borderLight}`, paddingTop: 10, marginBottom: 12 }}>
                                    ...
                                    {/* Individual bag weights — all batches side-by-side in a row.
                                        Each batch is a column showing its weights vertically.
                                        Saves space when there are many batches/commodities. */}
                                    {c.batches.some(b => b.individualWeights && b.individualWeights.length > 0) && (
                                        <div style={{
                                            ...
                                            {c.batches.map((b, j) => (
                                                <div key={j} style={{
                                                    flex: '1 1 0', minWidth: 80, padding: '8px 10px',
                                                    background: palette.bgSub || '#f8fafc', borderRadius: 6,
                                                    border: `1px solid ${palette.borderLight}`,
                                                }}>
                                                    <div style={{
                        fontSize: '0.7rem', color: palette.primary, fontWeight: 700, marginBottom: 6,
                        paddingBottom: 4, borderBottom: `1px solid ${palette.border}`,
                    }}>
                                                        Batch {j + 1}: {b.bagCount} bags
                                                    </div>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                                        {b.individualWeights?.map((w, k) => (
                                                            <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                                <span style={{ fontSize: '0.66rem', color: palette.muted, fontWeight: 600, minWidth: 20 }}>#{k + 1}</span>
                                                                <input type="number" inputMode="decimal" step="0.001" min="0"
                                                                    value={w}
                                                                    onChange={(e) => updateIndividualWeight(i, j, k, e.target.value)}
                                                                    disabled={billed}
                                                                    style={{
                                                    ...
                                                    <div style={{
                                                        ...
                                                        fontSize: '0.7rem', color: palette.muted, fontWeight: 600,
                                                    }}>
                    ...
                    <div style={{ marginTop: 20, padding: 16, background: palette.white, border: `1px solid ${palette.borderLight}`, borderRadius: 12, boxShadow: SHARED.shadowMd }}>
                        ...
                        {/* Payment mode selector — Cash / UPI / Bank / Due */}
                        {!billed && (
                            <div style={{ marginBottom: 12, padding: '10px 12px', background: palette.white, border: `1.5px solid ${palette.border}`, borderRadius: 10 }}>
                                <label style={{ display: 'block', fontSize: '0.78rem', color: palette.muted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
                                    Payment Mode
                                </label>
                                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                    {([['cash', '💵 Cash'], ['upi', '📱 UPI'], ['bank', '🏦 Bank'], ['due', '📋 Due (Credit)']] as const).map(([mode, label]) => (
                                        <button
                                            key={mode}
                                            onClick={() => setPaymentMode(mode)}
                                            style={{
                                                flex: '1 1 100px', padding: '8px 12px', borderRadius: 8,
                                                background: paymentMode === mode ? palette.primary : palette.bgSub || '#f1f5f9',
                                                color: paymentMode === mode ? '#fff' : palette.text,
                                                border: `1px solid ${paymentMode === mode ? palette.primary : palette.border}`,
                                ...
                                {paymentMode === 'due' && (
                                    <p style={{ margin: '8px 0 0', fontSize: '0.74rem', color: palette.muted }}>
                                        📋 Buyer will pay in installments. After saving, track partial payments on the ledger page.
                                        Remaining: <strong style={{ color: palette.primary }}>{formatINR(grandTotalAmount)}</strong>
                                    </p>
                                )}
                            </div>
                        )}

                        {/* Lock banner — shown after bill is saved/printed/shared */}
                        {billed && (
                            <div style={{ marginBottom: 12, padding: '10px 14px', background: '#d1fae5', border: '1.5px solid #6ee7b7', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: '1.1rem' }}>🔒</span>
                                <div>
                                    <p style={{ margin: 0, fontSize: '0.84rem', fontWeight: 700, color: '#065f46' }}>Bill locked — cannot be edited</p>
                                    <p style={{ margin: '2px 0 0', fontSize: '0.72rem', color: '#047857' }}>
                                        Payment mode: {paymentMode.toUpperCase()}
                                        {paymentMode === 'due' && ' · Track installments on ledger page'}
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                            ...
                            >📤 Share</button>
                            <button onClick={saveToLedger} disabled={saving || grandTotalAmount <= 0 || billed}
                                style={{ flex: 1, minWidth: 130, padding: '12px 16px', background: saving || grandTotalAmount <= 0 || billed ? palette.muted : palette.primary, color: '#fff', border: 'none', borderRadius: 10, fontSize: '0.88rem', fontWeight: 700, cursor: saving || grandTotalAmount <= 0 || billed ? 'not-allowed' : 'pointer' }}
                            >{billed ? '✓ Saved' : saving ? 'Saving…' : '💾 Save'}</button>
