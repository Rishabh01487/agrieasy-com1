...
<style>
    * { box-sizing: border-box; }
    ...
    @media print { body { padding: 0; } .receipt { border: none; max-width: 100%; } .no-print { display: none; } }
</style>
</head>
<body>
<div class="receipt">
                    ...
                    <div style={{ marginTop: 20, padding: 16, background: palette.white, border: `1px solid ${palette.borderLight}`, borderRadius: 12, boxShadow: SHARED.shadowMd }}>
                        ...
                        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                            <button onClick={printReceipt} disabled={grandTotalAmount <= 0}
                                style={{ flex: 1, minWidth: 140, padding: '12px 20px', background: grandTotalAmount <= 0 ? palette.muted : '#123C69', color: '#fff', border: 'none', borderRadius: 10, fontSize: '0.92rem', fontWeight: 700, cursor: grandTotalAmount <= 0 ? 'not-allowed' : 'pointer' }}
                            >🖨️ Print Receipt</button>
                            <button onClick={saveToLedger} disabled={saving || grandTotalAmount <= 0}
                                style={{ flex: 1, minWidth: 140, padding: '12px 20px', background: saving || grandTotalAmount <= 0 ? palette.muted : palette.primary, color: '#fff', border: 'none', borderRadius: 10, fontSize: '0.92rem', fontWeight: 700, cursor: saving || grandTotalAmount <= 0 ? 'not-allowed' : 'pointer' }}
                            >{saving ? 'Saving…' : '💾 Save to Ledger'}</button>
                            <button onClick={resetAll}
                                style={{ padding: '12px 20px', background: palette.white, color: palette.text, border: `1.5px solid ${palette.border}`, borderRadius: 10, fontSize: '0.92rem', fontWeight: 700, cursor: 'pointer' }}
                            >📸 New Bill</button>
                        </div>
                        {saveMsg && (
