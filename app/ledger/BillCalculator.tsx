...
Generated by AgriEasy · Jai Jawan, Jai Kisan 🇮🇳`
    ...
    const resetAll = () => {
        setResult(null); setFile(null); setPreviewUrl(''); setRates({}); setSaveMsg(''); setError(''); setBillPhotoUrl(''); setInputMode('photo')
    }

    // ── Inline content (shared between embedded + standalone) ──
    const content = (
        <>
            {/* Mode toggle — Photo vs Manual Entry */}
            {!result && (
                <div style={{
                    display: 'flex', gap: 4, padding: 4, background: palette.bgSub || '#f1f5f9',
                    borderRadius: 12, marginBottom: 16,
                }}>
                    <button
                        onClick={() => { setInputMode('photo'); setError('') }}
                        style={{
                            flex: 1, padding: '10px 16px', borderRadius: 9,
                            background: inputMode === 'photo' ? palette.white : 'transparent',
                            color: inputMode === 'photo' ? palette.primary : palette.muted,
                            border: 'none', fontSize: '0.88rem', fontWeight: 700,
                            cursor: 'pointer', transition: 'all 0.2s ease',
                            boxShadow: inputMode === 'photo' ? SHARED.shadow : 'none',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                        }}
                    >📸 Photo</button>
                    <button
                        onClick={() => { setInputMode('manual'); setError(''); setFile(null); setPreviewUrl('') }}
                        style={{
                            flex: 1, padding: '10px 16px', borderRadius: 9,
                            background: inputMode === 'manual' ? palette.white : 'transparent',
                            color: inputMode === 'manual' ? palette.primary : palette.muted,
                            border: 'none', fontSize: '0.88rem', fontWeight: 700,
                            cursor: 'pointer', transition: 'all 0.2s ease',
                            boxShadow: inputMode === 'manual' ? SHARED.shadow : 'none',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                        }}
            ...
            {!result && inputMode === 'manual' && (
                <ManualBillEntry
                    onGenerate={(manualResult) => {
                        // Pick up rates + counterparty that ManualBillEntry stashed on window
                        const manualRates = (window as any).__manualBillRates
                        const manualCounterparty = (window as any).__manualBillCounterparty
                        if (manualRates) setRates(manualRates)
                        if (manualCounterparty) setCounterpartyName(manualCounterparty)
                        setResult(manualResult)
                    }}
                />
            )}

            {/* Photo mode — upload area + camera buttons */}
