                ...
                const individualHtml = b.individualWeights && b.individualWeights.length > 0
                    ? `<tr class="batch-detail-row">
                            <td colspan="4" style="padding: 4px 12px 4px 28px; background: #FAFAF5;">
                                <div style="font-size: 10px; color: #6B6B6B; line-height: 1.8; font-family: monospace;">
                                    ${b.individualWeights.map((w, k) => {
                        const bagNum = globalBagNum + k
                        return `<div style="padding: 1px 0;">Bag ${bagNum}: <strong style="color: #2A2A2A;">${w.toFixed(3)}</strong> kg</div>`
                    }).join('')}
                                </div>
                                                        ...
                                                        {b.individualWeights.map((w, k) => {
                                                            ...
                                                            return (
                                                                <div key={k} style={{
                                                                    ...
                                                                    fontFamily: 'monospace', whiteSpace: 'nowrap', padding: '1px 0',
                                                                }}>
                                                                    <span style={{ color: palette.muted }}>Bag {globalBagNum}:</span>{' '}
                                                                    <strong style={{ color: palette.text }}>{w.toFixed(3)}</strong> kg
                                                                </div>
                                                            )
                                                        })}
                                                    </div>
